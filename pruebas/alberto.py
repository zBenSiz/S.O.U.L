...
class FantasmaAssistant(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
    ...
    def listen(self):
        ...
        try:
            with sr.Microphone() as source:
                print('Escuchando...')
                voice = self.listener.listen(source, timeout=3)  # Escuchar durante 4 segundos
                recognizer = self.listener.recognize_google(voice, language='es-MX')
                recognizer = recognizer.lower()

                # Verificar si la palabra "fantasma" está presente en el comando
                if 'fantasma' in recognizer:
                    # Remover la palabra "fantasma" del comando
                    recognizer = recognizer.replace('fantasma', '')

                    # Imprimir el comando reconocido para verificar
                    print("Recognizer:", recognizer)

                    return recognizer  # Solo retorna el comando si incluye "fantasma"
                else:
                    print("Comando no incluye 'fantasma'")
        except Exception as e:
            print('Algo ha salido mal:', str(e))
        return ""  # Retorna una cadena vacía si no se detectó "fantasma" en el comando

    # Método que ejecuta al asistente
    def run(self):
        global saludo_inicial

        # Bandera para mantener el bucle de escucha activo
        listening = True
        ...
        while listening:
            ...
            # Aquí puedes agregar más condiciones según las necesidades

            # Si deseas salir del bucle de escucha, puedes usar un comando específico, por ejemplo:
            if 'detente' in recognizer:
                self.talk('Adiós. Hasta luego')
                listening = False
